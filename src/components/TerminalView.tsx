...
export function TerminalView({ onOpenBrowser }: Props) {
  const { recordCommand, snapshot, addCredits } = useGame();
  ...
  function run(cmdRaw: string) {
    ...
    if (head === 'help') {
      pushOut(
        'help clear echo open time joke sudo ping credits stats fortune quests sites whoami',
      );
      recordCommand();
      return;
    }
    ...
    if (head === 'sites') {
      pushOut('rn:home search directory forum mart mail weather radio wiki jobs chat hack map arcade quests status readme');
      recordCommand();
      return;
    }
    if (head === 'whoami') {
